...
class AIScraper:
    """
    Extractor class that uses AI to extract information from web pages.
    ...
    def scrape(self, schema: dict , tags_to_extract: list[str] = ["span", "p", "h1", "h2", "a"]) -> dict:
        """
    ...
    def scrape_and_save(self, schema: dict ,
                        tags_to_extract: list[str] = ["span", "p", "h1", "h2", "a"],
                        filename: str = "extracted",
                        dir: str = "",
                        format: str = "json"):
        """
        Shortcut to scrape and save the data to a file
        ...
        """
        data = self.scrape(schema, tags_to_extract)
        if format == "json":
            save_json(data, filename, dir)
        elif format == "csv":
            save_csv(data, filename, dir)
        else:
